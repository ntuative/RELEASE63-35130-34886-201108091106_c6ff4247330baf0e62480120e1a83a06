package com.sulake.habbo.ui.widget.avatarinfo
{
   ...
   public class AvatarMenuView extends AvatarInfoView
   {
      ...
      protected static const const_1049:uint = 1;
      
      protected static const const_1050:uint = 2;
      
      protected static var var_1450:uint = const_1049;
      
      private static const const_1563:uint = 16744755;
      
      private static const const_1564:uint = 16756591;
      
      private static const const_1098:int = 8;
       
      
      protected var _data:AvatarInfoData;
      
      protected var var_657:uint = 1;
      ...
      protected function updateButtons() : void
      {
         ...
         if(!this.var_1730)
         {
            ...
            {
               this.var_657 = const_1050;
            }
            else
            {
               this.var_657 = var_1450;
            }
         }
         if(this.var_657 == const_1049)
         {
      ...
      private function showButton(param1:String, param2:Boolean = true) : void
      {
         ...
         if(_loc6_)
         {
            ...
            {
               _loc6_.x = _loc5_.x + (_loc5_.width - _loc5_.textWidth) / 2 - _loc6_.width - const_1098;
            }
            if(_loc6_.tags.indexOf("arrow_right") != -1)
            {
               _loc6_.x = _loc5_.x + (_loc5_.width + _loc5_.textWidth) / 2 + const_1098;
            }
         }
      }
      ...
      private function buttonEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         else if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER)
         {
            if(param2.name == "button")
            {
               param2.color = param2.tags.indexOf("moderate") > -1 ? uint(const_2101) : uint(const_1051);
            }
            else if(param2.tags.indexOf("link") > -1)
            {
               if(param2.tags.indexOf("actions") > -1)
               {
                  ITextWindow(IWindowContainer(param2).getChildAt(0)).textColor = const_1446;
               }
               else if(param2.tags.indexOf("moderate") > -1)
               {
                  ITextWindow(IWindowContainer(param2).getChildAt(0)).textColor = const_1564;
               }
            }
         }
         else if(param1.type == WindowMouseEvent.const_25)
         {
            if(param2.name == "button")
            {
               param2.color = const_1448;
            }
            else if(param2.tags.indexOf("link") > -1)
            {
               if(param2.tags.indexOf("actions") > -1)
               {
                  ITextWindow(IWindowContainer(param2).getChildAt(0)).textColor = const_1447;
               }
               else if(param2.tags.indexOf("moderate") > -1)
               {
                  ITextWindow(IWindowContainer(param2).getChildAt(0)).textColor = const_1563;
               }
            }
         }
         if(_loc3_)
