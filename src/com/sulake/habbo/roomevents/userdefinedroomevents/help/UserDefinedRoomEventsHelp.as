package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1925:IWindowContainer;
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_94.getXmlWindow("ude_help"));
         this.var_1925 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1258 = this.var_1925.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1258)
         {
            this.var_1258.procedure = this.onButtonClick;
         }
         this.var_1259 = this.var_1925.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1259)
         {
            this.var_1259.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_284 >= this.var_73.length - 1)
         {
            this.var_1259.disable();
         }
         else
         {
            this.var_1259.enable();
         }
         if(this.var_284 <= 0)
         {
            this.var_1258.disable();
         }
         else
         {
            this.var_1258.enable();
         }
      }
      ...
      {
         ++this.var_284;
         this.var_284 = Math.min(this.var_284,this.var_73.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_284;
         this.var_284 = Math.max(this.var_284,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this.var_73 = [];
         var _loc1_:int = 1;
         while(this.var_94.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_94.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_94.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_94.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_73[this.var_284][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_73[this.var_284][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_73[this.var_284][1]).value;
      ...
      {
         if(!param1 || !this.var_94 || !this.var_94.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_94.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_94.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_94.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
