package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1869:int = 4;
      ...
      {
         return this.var_483.length + this.var_312.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_312.length < const_1869 && this.var_483.length > 0)
            {
               _loc1_ = this.var_483.shift();
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_483.indexOf(loader);
            if(index > -1)
            {
               this.var_483.splice(index,1);
            }
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_483)
            {
               if(param2)
               {
