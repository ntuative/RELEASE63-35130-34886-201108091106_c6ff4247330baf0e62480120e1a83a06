package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_2106:uint = 2;
      ...
      public static const const_1795:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1291:Boolean = false;
      
      public static const const_1869:int = 2;
      
      private static var var_1102:Array = new Array();
      ...
      protected var var_738:Class;
      
      protected var var_523:uint = 0;
      ...
      protected var var_3092:String = "";
      
      protected var var_2298:int;
      
      protected var var_1444:int;
      
      protected var var_1445:int;
      
      protected var var_3159:uint = 0;
      
      protected var var_3093:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         if(const_1291)
         {
            if(var_1102.indexOf(param1) == -1)
            {
               var_1102.push(param1);
            }
            throttle();
         }
         else
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1291)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_888[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_888.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_888.length < const_1869 && false)
            {
               _loc1_ = var_1102.shift() as LibraryLoader;
      ...
      {
         return !!this.var_1171 ? uint(this.var_1444 - this.var_2298) : uint(getTimer() - this.var_2298);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_436,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_44.close();
            }
      ...
      {
         if(this.var_1170 && !_disposed)
         {
            this.var_1170 = false;
            if(!this.var_1171 && this.var_267)
            {
               this.var_1444 = -1;
               this.var_2298 = getTimer();
               queue(this);
            }
         }
      }
      ...
      {
         if(!this.var_1171 && !_disposed && this.var_1445 > 0)
         {
            try
            {
               this.var_44.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_267,const_1795 - this.var_1445);
            if(this.var_3093)
            {
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_436 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_436,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_436,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_436))
               {
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_523 = LibraryLoader.name_5;
         ...
         if(this.var_523 == LibraryLoader.const_2105)
         {
         ...
         if(this.var_523 == LibraryLoader.name_5)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_436,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_738 = this.getDefinition(this._name) as Class;
         ...
         try
         {
            xmlClass = this.var_738.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_436,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_436,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
